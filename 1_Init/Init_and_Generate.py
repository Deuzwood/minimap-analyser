...
with open(const.CHAMPIONS_LIST_PATH, 'r') as json_file:
    ...
    print("Nombre de champions : "+str(len(_list)))
...
for mapIndex in range(nbGeneratedMap):
    ...
    for randomChampIndex in randomList:
        ...
        if( _list[randomChampIndex] != "Yuumi" or lastBlue[0] == -1 or random.random() > 0.4 ):
            ...
            if (redBool == True):
                ...
                rift.paste(yuumi_blue, (lastBlue[0]-3, lastBlue[1]-6),yuumi_blue)
                redBool = True
                x = lastBlue[0]
                y = lastBlue[1]
            print(str(mapIndex))
        row+=' '+str(x)+','+str(y)+','+str(x+26)+','+str(y+26)+","+str(randomChampIndex)
